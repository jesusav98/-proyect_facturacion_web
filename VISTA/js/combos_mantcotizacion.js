...
$('#buscar_porfecha').change(function() {
	var fecha=0
	if( $('#buscar_porfecha').prop('checked') ) {
		fecha=1
	}
	cargarCotizacion(fecha)
});
...
function cargarCotizacion(fecha){
	...
	if(fecha>=0){
		$.ajax({
			url: URL
				...
				$("#tabla_consultacotizacion tr").remove()
				$('#tabla_consultacotizacion').html(data); 
		});
		
	}
} 

function mostrarCotizacion(numdoc){
	var url="editar_cotizacion.php?numdoc="+numdoc
	$.ajax({
		url: url 
	}).done(function(data) {
			$('#editar_cotizacion').html(data); 
			$('#mant_cotizacion').modal("show")
	});
}

function cargarDatos(NUMDOC,DIRCLI,fecha,CODVEN,TIPVEN,CODMON){
	mostrar_Vendedor() 
	mostrar_forma_pago() 
	mostrar_moneda() 
	mostrar_direccion_cliente()
	$("#Fec_doc").val(new Date(fecha).getDate())
	$("#Vendedor").val(CODVEN)
	$("#forma_pago").val(TIPVEN)
	$("#moneda option[value='"+ CODMON +"']").attr("selected",true)
	$("#Lugar_entrega").val(DIRCLI)
	mostrar_detalle_cotizacion(NUMDOC)
}
...
function mostrar_detalle_cotizacion(NUMDOC) {
	$('#Lugar_entrega').empty()
	RUTA='../CONTROLLER/controller_detallecotizacion.php?opc_DetalleCotizacion=2&NUMDOC='+NUMDOC
	$.get(RUTA, function (data) {
		$(data).each(function(row) {
			//var DIRENT = data[row].DIRENT; 
			//$('#Lugar_entrega').append(resultado);
...
function actualizarCortizacion(NUMDOC){
	var codigo=$("#codigo").val()
	...
	var dir='../CONTROLLER/controller_cotizacion.php?opc_Cotizacion=2&NUMDOC='+NUMDOC+'&codigo='+codigo+'&Raz_social='+Raz_social+'&Lugar_entrega='+Lugar_entrega+'&ruc='+ruc+'&actencion='+actencion
	...
	$.ajax(
		{
			...
			success: function( data ) {
				...
				$('#tabla_producto tr').each(function () {
					...
					if(SERVIC!="SERVIC"){
						NUMITE+=1;
						var dirDEt='../CONTROLLER/controller_detallecotizacion.php?opc_DetalleCotizacion=3&numdoc='+NUMDOC+'&NUMITE='+NUMITE+'&IDPROD='+IDPROD+'&DESITE='+DESITE+'&cantidad='+cantidad
								...
								success: function( data ) {
									if (data>0){
										...
										if(NUMITE==cantidadItem){
											alert("Cotizacion "+NUMDOC +" actualziada correctamente")
											//location.reload(true)
											$("#generar-cotizacion").modal('hide')
										}
									}else{
...
function aceptarCotizacion(NUMDOC){
	var dir='../CONTROLLER/controller_cotizacion.php?opc_Cotizacion=3&NUMDOC='+NUMDOC
	$.ajax(
		{
			url: dir,
			success: function( data ) {
				if (data>0){
					...
					alert("Eror al aprobar cotizacion")
				}
			}
		});
}

function imprimir(numdoc){
